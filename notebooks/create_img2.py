...
def fdr(img, q=.1):
    ...
    thrline = idx*q/(V*cV)
    select = pval <= thrline
    print 'num voxels below line:', np.sum(select), 'out of', len(img)

    if np.sum(select) == 0:
        thr = 10
    else:
        thr = np.min(zscores[::-1][select])

    ax.plot(pval[:100], '-')
    ax.plot(thrline[:100], '-')
    ax.plot([0, 100], [0.001, 0.001], '-')
    ...
    def __init__(self,
        ...
        for i, contrast in enumerate(contrasts):
            ...
            if fdr_corrected:
                ...
                if masks:
                    include *= nib.load(masks[i]).get_data().astype(bool)

                data = img_con[include] * -1 if neg else img_con[include]
                th = fdr(data) * -1 if neg else fdr(data)
            else:
                th = thresh[i]

            self.thresh.append(th)
            mask = img_con > -th if neg else img_con < th
            img_con[mask] = 0
...
if __name__ == '__main__':
    ...
    con2img = Con2Img('figure_6.png',
                      ...
                      cmaps=cmaps,
                      center=60,
                      masks=[os.path.join(root, 'onset_mask.nii.gz'),
